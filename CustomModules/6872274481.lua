...
run(function()
	...
	JellyfishExploit = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({Name="JellyfishExploit",Function=function(callback)
		if callback then
			task.spawn(function()
				repeat
					task.wait(0.2);
					local args = {[1]="electrify_jellyfish"};
	...
	end,HovorText="Requires Marina kit to use"});
end);
run(function()
	local HotbarMods = {};
	...
	local function hotbarFunction()
		local inventoryicons = ({pcall(function()
		...
		if (inventoryicons and (type(inventoryicons) == "userdata")) then
			for i, v in next, inventoryicons:GetChildren() do
				local sloticon = ({pcall(function()
					return v:FindFirstChildWhichIsA("ImageButton"):FindFirstChildWhichIsA("TextLabel");
				end)})[2];
	...
	HotbarMods = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({Name="HotbarMods",HoverText="Add customization to your hotbar.",Function=function(calling)
		if calling then
			task.spawn(function()
	...
	HotbarRounding = HotbarMods.CreateToggle({Name="Rounding",Function=function(calling)
		pcall(function()
			HotbarRoundRadius.Object.Visible = calling;
		end);
	...
	HotbarRoundRadius = HotbarMods.CreateSlider({Name="Corner Radius",Min=1,Max=20,Function=function(calling)
		for i, v in next, hotbarobjects do
			pcall(function()
				v.CornerRadius = UDim.new(0, calling);
			end);
		end
	end});
...
run(function()
	...
	local oldtime;
	local oldobjects = {};
	local themetable = {Custom=function()
	...
	Atmosphere = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({Name="Atmosphere",ExtraText=function()
		return ((AtmosphereMethod.Value ~= "Custom") and AtmosphereMethod.Value) or "";
	end,Function=function(callback)
		if callback then
	...
	AtmosphereMethod = Atmosphere.CreateDropdown({Name="Mode",List=themetab,Function=function(val)
		task.spawn(function()
	...
	SkyUp = Atmosphere.CreateTextBox({Name="SkyUp",TempText="Sky Top ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyDown = Atmosphere.CreateTextBox({Name="SkyDown",TempText="Sky Bottom ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyLeft = Atmosphere.CreateTextBox({Name="SkyLeft",TempText="Sky Left ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyRight = Atmosphere.CreateTextBox({Name="SkyRight",TempText="Sky Right ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyFront = Atmosphere.CreateTextBox({Name="SkyFront",TempText="Sky Front ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyBack = Atmosphere.CreateTextBox({Name="SkyBack",TempText="Sky Back ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkySun = Atmosphere.CreateTextBox({Name="SkySun",TempText="Sky Sun ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyMoon = Atmosphere.CreateTextBox({Name="SkyMoon",TempText="Sky Moon ID",FocusLost=function(enter)
		if Atmosphere.Enabled then
			Atmosphere.ToggleButton(false);
			Atmosphere.ToggleButton(false);
		end
	end});
	SkyColor = Atmosphere.CreateColorSlider({Name="Color",Function=function(h, s, v)
		if skyatmosphereobj then
			skyatmosphereobj.TintColor = Color3.fromHSV(SkyColor.Hue, SkyColor.Sat, SkyColor.Value);
		end
	end});
...
run(function()
	...
	RemoveKillFeed = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({Name="KillFeedHider",Function=function(callback)
		if callback then
			task.spawn(function()
	...
	end,HoverText="Removes KillFeed"});
end);
run(function()
	local GodMode = {Enabled=false};
	GodMode = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({Name="AntiHit (1b0c)",Function=function(callback)
		if callback then
			spawn(function()
				while task.wait() do
...
local Messages = {"nigger","PlaceRealm","placeholder:green_circle:!","subscribe","lua!"};
local Indicator = {Enabled=true};
Indicator = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({Name="Damage Indicator",Function=function(callback)
	if callback then
		old = debug.getupvalue(bedwars['DamageIndicator'], 10, {Create});
		debug.setupvalue(bedwars['DamageIndicator'], 10, {Create=function(self, obj, ...)
			spawn(function()
				pcall(function()
		...
		debug.setupvalue(bedwars['DamageIndicator'], 10, {Create=old});
		old = nil;
	end
end});
local ChatNuker = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({Name="ChatNuker",Function=function(callback)
	if callback then
		while true do
			wait(1.7);
			local args = {[1]=" ",[2]="All"};
...
end,Default=false,HoverText="breaks the chat"});
local ChatBypass = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({Name="Chat Bypass",HoverText="makes me cum",Function=function(callback)
	if callback then
...
run(function()
	local AutoBuyEra = {};
	AutoBuyEra = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({Name="AutoBuyEra",Function=function(calling)
		if calling then
			task.spawn(function()
				repeat
					task.wait();
					local args = {[1]={era="iron_era"}};
					...
					local args = {[1]={era="diamond_era"}};
					game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("RequestPurchaseEra"):InvokeServer(unpack(args));
					local args = {[1]={era="emerald_era"}};
					game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("RequestPurchaseEra"):InvokeServer(unpack(args));
					local args = {[1]={upgrade="altar_i"}};
					...
					local args = {[1]={upgrade="bed_defense_i"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="destruction_i"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="magic_i"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="altar_ii"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="destruction_ii"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="magic_ii"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
					local args = {[1]={upgrade="altar_iii"}};
					game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.RequestPurchaseTeamUpgrade:InvokeServer(unpack(args));
